...
-   **Some of the data is incomplete/there is some missing information in some of the columns. Some of the values like the course_id and the date contain multiple pieces of information that may need to be separated.**
...
-   **The table now shows both the number of STEM classes taken and the interest in STEM by the students in one dataset.**
...
-   **The grade book data set did not have information about how much time students spent on the course and in comparison, the time spent data set did not contain information about the students' points earned.**
...
-   **The data shows the mean/average score earned from the total points possible in the class and can also show the average time spent on the course in hours**
...
```{r}
ggplot(data_to_explore) +
  geom_histogram(aes(x = time_spent_hours), bins = 50)
```
...
-   **If one wanted to see a general, overall trend then it would be best to do a lower number such as 50 or 75. If one wanted to see a very specific value distribution then it would be best to do a higher number such as 100, 150, or even 200.**
...
-   **PhysA has very few students who spent 50+ hours in the course whereas a course like AnPhA had more students who spent 50-100+ hours in the course. BioA does not have many students who spent time in course across the board whereas FrScA had many more students spending time in the course.**
...
-   **Typically, students who spent about 25 hours in the course received a high amount of points but this was not always the case - some students spent hardly any (if at all) time in the course and still received high marks whereas students who spent 100+ hours also received high scores. However, there were plenty of students who did not spend a lot of time in the course and did not get a high score. Therefore, it could be said that students who spent more time got a higher score.**
-   **The code is simple and completes the task as needed.** **Perhaps color could be added to show the difference in the classes.**
...
-   **The following graph provides information about how a student's interaction time with an LMS impacts their final class score. As seen in the graph, for students who have been approved and enrolled in the class, there is a clear distinction that the more time spent in the class the higher the final score is. However, there are some outliers wherein students who do not spend much time in the course still receive a high grade.**
...
```{r}
data_to_explore |> 
  select(proportion_earned, time_spent_hours, tv, q7, q10) |> 
  correlate() |> 
  rearrange() |>
  shave() |>
  fashion()
...
```{r}
m2 <- lm(proportion_earned ~ time_spent + tv, data = data_to_explore)
...
-   **The p value is less than 0.5 which indicates that the two variables chosen, "time_spent" and "tv" are statistically significant and are related to points earned.**
